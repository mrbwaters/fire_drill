function collision(){
	if hsp == 0 then hsp_decimal = 0;
	if vsp == 0 then vsp_decimal = 0;
	...
	if hsp > 0 then side = bbox_right else side = bbox_left;
	...
	if	((t1 != VOID) and (t1 != PLATFORM)) or 
		...
		//collision found
		if (hsp > 0) then x = x - (x mod global.tile_size) + global.tile_size - 1 - (side - x);
	...
	var side;
	//determine which side to test
	if vsp > 0 then side = bbox_bottom else side = bbox_top;
	...
	if	(t1 != VOID and (((vsp > 0 or t1 != PLATFORM)) and t3 != PLATFORM) or (t1 == SOLID and t3 == PLATFORM)) or
		...
		//collision found
		if (vsp > 0) then y = y - (y mod global.tile_size) + global.tile_size - 1 - (side - y);
